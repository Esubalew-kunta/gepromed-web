...
export type TrainingSession = {
  ...
  accessibilityInfo?: L; // Accessibilité handicap
  accessibilityReferent?: string; // named disability-referent contact
  certificateDelivered?: L; // Certificat / attestation délivrée
  registrationInfo?: L; // Modalités et délais d'inscription
  priceNote?: L; // Tarif en texte libre, ou nom du financeur
  ...
  // Set once per training by Gepromed staff, never asked of the registrant.
  isSponsored?: boolean;
  sponsors?: { name: string; logoUrl?: string; website?: string; referentName?: string }[];
};
 ...
 * `format=pdf` makes the console return a real, downloadable PDF binary
 * (Content-Disposition: attachment) instead of the browser-print HTML page.
 */
export function programPdfUrl(slug: string): string | null {
  const base = process.env.NEXT_PUBLIC_PROGRAM_API_URL;
  if (!base) return null;
  return `${base.replace(/\/$/, "")}/api/programs?session=${encodeURIComponent(slug)}&format=pdf`;
}
